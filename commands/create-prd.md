...
- **External API Integrations:** For each API: name, version, documentation URL, purpose, authentication method, integration pattern, compatibility notes, validation strategy, research reference
- **Internal Endpoints:** Endpoint definitions, request/response formats, authentication requirements, example payloads
...
- **External API Integration Risks:** Include if APIs involved: API compatibility issues, SDK version incompatibility, undocumented behavior, rate limits, configuration issues
...
### 1.5. API and External Dependency Research

**Before creating the PRD, identify if external APIs or SDKs are involved:**

If the feature requires external APIs, services, or unfamiliar SDKs:

1. **Ask user about research documents:**
   ```
   Does this feature integrate with external APIs or SDKs?

   If yes:
   - Do you have existing research documents? (provide paths)
   - Should I research the APIs before creating the PRD? (y/n)
   - Which APIs/SDKs need research? (list)
   ```

2. **If research needed:**
   - Use `/core_piv_loop:explore-api [API name]` for each unfamiliar API
   - Wait for research completion before proceeding
   - Incorporate findings into PRD sections 8 (Tech Stack) and 10 (API Specification)

3. **If research documents provided:**
   - Read and incorporate into PRD
   - Reference in Section 10 (API Specification) and Appendix

**Do not proceed to PRD writing without:**
- ✓ User confirmation on research needs
- ✓ Completed API research (if needed)
- ✓ Existing research documents read (if provided)
